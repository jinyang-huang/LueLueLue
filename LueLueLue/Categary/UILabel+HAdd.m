...
+ (UILabel *)buildLabelWithFrame:(CGRect)frame backColor:(UIColor *)color content:(NSString *)text textColor:(UIColor *)tColor font:(id )font textAlignment:(NSTextAlignment)alignment lines:(NSInteger)line {
    ...
    if ([font isKindOfClass:[UIFont class]]) {
        ...
        NSNumber *n = font;
        targetFont = [UIFont systemFontOfSize:[n floatValue]];
    }
    UILabel *newLabel      = [self buildLabelWithFrame:frame backColor:color content:text textColor:tColor font:targetFont textAlignment:alignment];
...
- (void)ha_calculateWidthSingleLine {
    CGSize size = [self.text sizeWithFont:self.font maxSize:CGSizeMake(CGFLOAT_MAX, 25)];
